...
def trash_kg_get_data():
    df = pd.read_excel(file_name,
                       ...
                       skiprows=2,
                       usecols="A,ME,MF,MG")
    df.columns = ['日期','水肥+生廚餘(kg)','生廚餘(kg)','水肥(kg)']
...
def trash_car_get_data():
    df = pd.read_excel(file_name,
                       ...
                       skiprows=2,
                       usecols="A,MH,MI,MJ")
    df.columns = ['日期','水肥+生廚餘(車次)','生廚餘(車次)','水肥(車次)']
...
#自動水質監測(下載)---------------------------------------------------------------------
fig_auto_manual_compare = px.line(df_auto_manual_compare, x='日期', y=['COD','COD(下載)','ss','ss(下載)','氨氮','氨氮(下載)'],
                                  title="<b>手動監測與自動監測數值比較</b>",template="plotly_white")
fig_auto_manual_compare.update_layout(
    plot_bgcolor="rgba(0,0,0,0)",
    xaxis=(dict(showgrid=False)),
...
right_column.plotly_chart(fig_auto_manual_compare, use_container_width=True) 
...
fig_NaOCL_L = px.line(df_NaOCL_L, x='日期', y=['除臭大樓補充2A','固大除臭補充2B','消毒總用量',
                                               ...
                                               '除臭契約用量上限2.39ppm'],
                      title="<b>次氯酸鈉NaOCL</b>",template="plotly_white")
...
fig_trash_kg = px.line(df_trash_kg, x='日期', y=['水肥+生廚餘(kg)'], 
                       template="plotly_white")
...
#投肥車次
fig_trash_car = px.line(df_trash_car, x='日期', y=['水肥+生廚餘(車次)'],
                       template="plotly_white")
